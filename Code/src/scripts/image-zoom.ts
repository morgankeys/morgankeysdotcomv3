...
function setZoom(index: number): void {
  ...
  if (!activeDialog || !activeImage || !activeStage) return;

  levelIndex = Math.min(Math.max(index, 0), ZOOM_LEVELS.length - 1);
  activeImage.style.transform = `scale(${ZOOM_LEVELS[levelIndex]})`;
  ...
  requestAnimationFrame(() => {
    activeStage.scrollLeft = (activeStage.scrollWidth - activeStage.clientWidth) / 2;
    activeStage.scrollTop = (activeStage.scrollHeight - activeStage.clientHeight) / 2;
  });
}
